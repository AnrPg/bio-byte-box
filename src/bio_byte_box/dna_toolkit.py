...
def nucleotide_frequencies(sequence: str, is_dna: bool = True) -> dict[str, float]:
    ...
    if total_nucleotides == 0:
        return {nucleotide: 0 for nucleotide in (DNA_NUCLEOTIDES if is_dna else RNA_NUCLEOTIDES)}
    counts = count_nucleotides(sequence, is_dna)
    return {nucleotide: counts[nucleotide] / total_nucleotides for nucleotide in (DNA_NUCLEOTIDES if is_dna else RNA_NUCLEOTIDES)}